...
flowchart TD
    ...
    R["科学审查 Agent<br/>通过 / 修订一次 / 阻断"]
    F["本地门禁<br/>目的论 / 伪精确 / 证据越界"]
    G["DGX Spark 绘制<br/>Klein → FLUX.1"]
    V["图像连续性审查<br/>Step 多模态 + 技术门禁"]
    H["会话状态与四阶段回放"]
...
Step 3.7 Flash 把用户选择整理成严格 JSON，包括名称、性状、内部原因、外部压力、收益、代价、不确定性和图像提示。独立科学审查 Agent 随后读取草案、父代账本和证据包，可以放行、要求一次修订或阻断。本地门禁继续检查目的论、伪精确数字、语言和证据越界。

结构通过后，DGX Spark 上的 ComfyUI 才开始生成图片。候选图还要经过 Step 多模态连续性审查；图像能力不可用时，系统只保留技术检查，并把降级状态写清楚。图片可以决定纹理、颜色和局部外观，不能决定一条演化路线是否真实存在。知识层也不会给未知节点补造引用；找不到对应证据时，它返回 `no_match`。
...
科学审查 Agent 已经进入主链，并且有权在绘图前阻断。下一步不是再增加角色，而是用固定样本持续评测它的裁决；独立生物 RAG、生态模型和实验数据则可以把部分世界推向更细的参数化模拟。
...
接下来，文件化知识层会继续生长为独立生物 RAG 和证据账本；已经接入的科学审查与图像审查会补固定样本评测；有限类群还可以接入种群遗传、生态模型或实验数据。十天做成的是第一层产品，也是后续世界扩张的底座。
...
- 主要模型：Step 3.7 Flash、StepAudio 2.5、FLUX.2 Klein 4B、FLUX.1、HunyuanVideo-1.5
- 项目协议：MIT License
